...
import com.zippy.infra.storage.service.StorageService;
import com.zippy.infra.storage.service.impl.DefaultStorageService;
import com.zippy.infra.storage.spi.StorageBackend;
import com.zippy.infra.storage.spi.impl.LocalStorageBackend;
import com.zippy.infra.storage.spi.impl.S3StorageBackend;
import com.zippy.infra.storage.support.StorageObjectKeyGenerator;
...
@AutoConfiguration
@ConditionalOnProperty(prefix = "zippy.storage", name = "enabled", havingValue = "true", matchIfMissing = true)
...
public class StorageAutoConfiguration {
    ...
    private StorageBackend createS3StorageBackend(StorageProperties properties, ObjectProvider<S3Client> s3ClientProvider) {
        ...
        throw new IllegalStateException("No storageS3Client bean available for S3 storage backend. "
                + "Configure S3 credentials via zippy.infra.storage.s3.access-key/secret-key, "
                + "or provide a custom S3Client bean named 'storageS3Client'.");
    }

    @Bean("storageS3Client")
    @ConditionalOnProperty(prefix = "zippy.infra.storage", name = "type", havingValue = "S3")
