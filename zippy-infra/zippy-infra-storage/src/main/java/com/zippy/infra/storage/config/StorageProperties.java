...
@Data
@ConfigurationProperties(prefix = "zippy.storage")
public class StorageProperties {
    ...
    public void validate() {
        if (!StringUtils.hasText(datePathPattern)) {
            throw new IllegalStateException("zippy.storage.date-path-pattern must not be blank");
        }
        try {
            DateTimeFormatter.ofPattern(datePathPattern.trim());
        } catch (IllegalArgumentException | DateTimeParseException exception) {
            throw new IllegalStateException("Invalid zippy.storage.date-path-pattern: " + datePathPattern, exception);
        }
        if (maxInMemoryReadBytes <= 0) {
            throw new IllegalStateException("zippy.storage.max-in-memory-read-bytes must be greater than 0");
        }
    ...
    private void validateLocal() {
        if (local == null || !StringUtils.hasText(local.getRootPath())) {
            throw new IllegalStateException("zippy.storage.local.root-path must not be blank");
        }
    }

    private void validateS3() {
        if (s3 == null || !StringUtils.hasText(s3.getBucket())) {
            throw new IllegalStateException("zippy.storage.s3.bucket must not be blank");
        }
        boolean hasAccessKey = StringUtils.hasText(s3.getAccessKey());
        boolean hasSecretKey = StringUtils.hasText(s3.getSecretKey());
        if (hasAccessKey != hasSecretKey) {
            throw new IllegalStateException("Both zippy.storage.s3.access-key and secret-key must be configured together");
        }
    }
