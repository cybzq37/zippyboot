...
import com.zippy.infra.storage.config.StorageAutoConfiguration;
...
class StorageAutoConfigurationTest {
    ...
    void shouldFailFastWhenS3CredentialsArePartial() {
        contextRunner
                .withPropertyValues(
                        "zippy.storage.type=S3",
                        "zippy.storage.s3.access-key=test-access-key"
                )
                .run(context -> {
                    assertThat(context).hasFailed();
                    assertThat(context.getStartupFailure())
                            .hasMessageContaining("Both zippy.storage.s3.access-key and secret-key must be configured together");
                });
    }
    ...
    void shouldUseDedicatedStorageS3ClientBean() {
        contextRunner
                .withPropertyValues("zippy.storage.type=S3")
